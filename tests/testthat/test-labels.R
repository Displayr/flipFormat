...
test_that("Labels",
          {
              ...
              expect_equal(as.character(Labels(names(phone),phone)), as.character(unlist(sapply(phone, function(x) attr(x, "label")))))
              # A few missing labels
              phone <- phone[,1:6]
              attr(phone[, 1], "label") <- NULL
              attr(phone[, 3], "label") <- NULL
              expect_equal(Labels(names(phone),phone), c("id", "Does respondent have a mobile phone?", "q2", "Occupation", "Age",  "Top of mind awareness"))
              ...
              expect_equal(Labels(factor.coefficient.names, cola), c("(Intercept)", "Q3. Age: 25 to 29", "Q3. Age: 30 to 34", "Q3. Age: 35 to 39", "Q3. Age: 40 to 44", "Q3. Age: 45 to 49", "Q3. Age: 50 to 54", "Q3. Age: 55 to 64", "Q3. Age: 65 or more"))
              # Regression.
