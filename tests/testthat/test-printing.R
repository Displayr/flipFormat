...
test_that("Escaping HTML", {
    test.table <- data.frame("First Column" = c("<b>Hello</b>", "<b>darkness</b>", "<b>my old friend</b>"),
                             "Second Column" = c("I've come to", "talk with you", "<b>again</b>"),
                             stringsAsFactors = FALSE)
    expect_error(DataTableWithRItemFormat(test.table, escape.html = FALSE), NA)
    expect_error(DataTableWithRItemFormat(test.table, escape.html = c(-1,-3)), NA)
})
